...
for file in os.scandir(os.path.join(os.getcwd(), "raw_data", "code2DFD")):
    if not file.path.endswith("-code2DFD.json"):  # Get the json format
        continue
    file_path = file.path
    ...
    del g["directed"]
    del g["graph"]
    name = file.name.replace("-code2DFD.json", "")
